...
// import * from './heatmap/components/Heatmap';
...
import BedTimeGraph from './creepy/BedTimeGraph'
...
import Profile from "./video_profile.mp4";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// todo https://echarts.apache.org/examples/en/editor.html?c=circle-packing-with-d3
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const sensitive_bubbles = (data) => {
  let numEntries = data ? data.length : 0
  let data_is_there = data ? data['outer'].length : 0

  if (numEntries == 0 || data_is_there == 0) {
    return (

            <Box align="center" pad="large"> <Spinner_grommet border={[{ side: 'all', color: 'transparent', size: 'medium' },{ side: 'horizontal', color: 'brand', size: 'medium' }]}/> </Box>

      )
  } else {


    console.log("pandas24" + JSON.stringify(data))
    ...
    const temp_data234 = [{
          ...
          {
              name: "Loans",
              trackers: "- google.com <br/> - other.com",
              value: 5334.5
          }, {
              name: "Credit Cards",
              value: 566
    ...
    const options = {
        title: {
            useHTML: true,
            text: `Trackers learn your <i>potentially</i> sensitive interests`
        },
        subtitle: {
          useHTML: true,
          text: document.ontouchstart === undefined ?
            'Click on an inner circle to see how a tracker views it' : 'Tap on an inner circle to see how a tracker views it'
        },
        chart: {
          // height: 1000,
          // marginTop: 30,
...
const sensitive_bubbles_v2 = (data) => {
  ...
  if (numEntries == 0 || data_is_there == 0 || data_deep_check == 0) {
    ...
    let engagement_str = '<p style="font-size: 20;"> Go-To Late-Night Interest: <strong>' + "(((nothing yet)))" + '</strong> (<em>happened ' + "(((nothing yet)))" + " " + "(((nothing yet)))" + '</em>)</p>'
    ...
    const options = {
        ...
        tooltip: {
            ...
            formatter: function() {
              ...
              let seen_it = []
              if (this_entry.title) {
                ret += "<b>" + '<span style="color:' + this.point.color + '">' + String(this.series.name.split('/')[(this.series.name.split('/')).length-1]) + '</span>' + "  Webpages Visited" + "</b>" 
                ret += "<ul>"
    ...
    return (
      <ReactTable
        ...
        columns={[
          ...
          {Header: 'Trackers', 
            accessor: d => d.tracker_info.map((line)=><div>{line}</div>),
            id: 'tracker_count',
            Cell: row => (
              <div style={{ textAlign: "left" }} key={row.value}>
                  {row.value}
              </div>)
          },
          {Header: 'Likely time to visit',
            accessor: d => d.time_of_day_highest,
            id: 'time_visits',
            Cell: row => (
              <div style={{ textAlign: "center" }} key={row.value}>
                  {row.value}
              </div>)
          },
          {Header: 'No. visits',
            accessor: d => d.webpage_visits,
            id: 'num_visits',
            Cell: row => (
              <div style={{ textAlign: "center" }} key={row.value}>
                  {row.value}
...
const sensitive_info_pie = (slice, data) => {
  ...
  let numEntries = data ? data.length : 0
  let actual_data = data ? data.all_pie.outer_all.length : 0
  if (numEntries == 0 || actual_data == 0) {
    return (<Box align="center" pad="large"> <Spinner_grommet border={[{ side: 'all', color: 'transparent', size: 'medium' },{ side: 'horizontal', color: 'brand', size: 'medium' }]}/> </Box>)
  }
  else {
      ...
      let data_to_render;
      if (slice == "day_pie") {
        data_to_render = data.day_pie
      }
      if (slice == 'week_pie') {
        data_to_render = data.week_pie 
      }
      if (slice == 'month_pie') {
        data_to_render = data.month_pie
      }
      if (slice == 'all_pie') {
        data_to_render = data.all_pie
      }
    ...
    const options = {
        ...
        subtitle: {
          text: document.ontouchstart === undefined ?
            'Click on a slice for a breakdown' : 'Pinch the chart to zoom in'
        },
        chart: {
            // zoomType: "xy",
            marginTop: 50,
...
// BESTBESTBESTBESTBEST  https://jsfiddle.net/nathanReitinger/gc6juo3d/49/

// todo https://www.highcharts.com/docs/stock/range-selector
// have time on bottom with null values for no search 
// have mulitple "series" for searches on jobs (showing spike) and releationships(showing spikes) 


// todotodo
// use gradient: https://jsfiddle.net/nathanReitinger/gc6juo3d/6/
// use x axis from bedtimes
// have mulitple series depending on stream (job, relationship, other sensitive categories) 
// use y axis from sensitive showing time spent searching
/////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const options = {
        ...
        tooltip: {
            ...
            formatter: function() {
              ...
              let seen_it = []
              if (title) {
                ret += "<b>" + '<span style="color:' + this.point.color + '">' + this.series.name + '</span>' + "  Webpages Visited" + "</b>" 
                ret += "<ul>"
                for (let t of title) {
...
const searchesString = (data, type) => {
  let numEntries = data ? data.length : 0
  if (numEntries === 0) {     
    return ("")
  } else {

    console.log("bottle " + JSON.stringify(data))
    
    let to_render = ""
    let renderable = data.map(entry => String(entry.title) + type)
    renderable.forEach(function (item, index) {
      to_render += item + "\n"
    });
    // renderable = renderable.replaceAll(",", "\n")
...
{   
    ...
    let minutes = parseInt(String(ret).split(':')[0])
    if (minutes > 1) {
      minutes_ref += " minutes and "
    } if (minutes == 1) {
      minutes_ref += " minute and "
    } if (minutes == 0) {
      minutes = ''
    }

    let seconds_ref = ''
    let seconds = parseInt(String(ret).split(':')[1])
    if (seconds > 1) {
      seconds_ref += " seconds "
    } if (seconds == 1) {
      seconds_ref += " second "
    ...
    return (minutes + minutes_ref + seconds + seconds_ref)

    // if (String(ret).split(':')[0] == "0") {
    //   return (String(ret).split(':')[1] + " seconds")
    // } else {
    //   return (String(ret) + " minutes")
    // }
    // console.warn(ret)
...
function set_combine(tally, vote, index, arr) {

  for (var interest of Object.keys(vote)) {
    let time = vote[interest]
    if (interest in tally) {
      tally[interest] += time
    } else {
      tally[interest] = time
    }
  ...
  let sortable = Object.fromEntries(
      Object.entries(tally).sort(([,a],[,b]) => a-b).reverse()
  )
  ...
  var result = Object.keys(sortable).map((key) => [key, sortable[key]])
  console.log(result)
    ...
    return (
      ...
      for (let bin of bins) {
        ...
        if (count == 0) {
          ...
          entry = {min: min, max: max, color: '#' + hexColour, label: millisToReadable(max)}
        } 
        else {
...
let number_to_display = {6: "Sunday", 5: "Monday", 4: "Tuesday", 3: "Wednesday", 2: "Thursday", 1: "Friday", 0: "Saturday"}
...
const option = {
  ...
  tooltip: {
    ...
    // }
    return `${"<b>" + number_to_display[params.data[1]] + number_to_display_times[params.data[0]] + "</b>"}
            ${"<ul>" + params.data[3].grouped_interests.map(n=>"<li>" + n[0] + ": " + String(millisToReadable(n[1])).replaceAll(",", "") + "</li>").join('') + "</ul>"   }`;
    }
...
// value is total time during this hour
// .filter(n => n) removes duplicates
  // <Divider />
  // Total # of Records Binned: {heatmap.data.minutes.filter(r => r.data.date).length} // interest, minutes tracked 


                // {heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => <ul><li>  <b>{entry.title}</b> <ul> <li> Inference: {entry.inference} </li>  <li> Time: {fancyTimeFormat(entry.count)} minutes  </li> <li> Visited on: {moment(entry.date).format("dddd, MMMM Do YYYY, h:mm a")} </li></ul> </li></ul> )  } 
                // {heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => <ul><li> Inference: {entry.inference} ({fancyTimeFormat(entry.count)} minutes) </li></ul> )  } 
                // {heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => <ul><li> Inference: {entry.inference} ({fancyTimeFormat(entry.count)}) <ul><li> visiting {entry.title} </li></ul> </li></ul> ) } 


//   {
//     "inference": "Business & Industrial",
//     "title": "Home - SmallNetBuilder",
//     "date": "2021-09-07T02:58:02.000Z",
//     "count": 4.781
//   },
//   {
//     "inference": "News",
//     "title": "new york post - Google Search",
//     "date": "2021-09-07T02:58:06.000Z",
//     "count": 1.903
//   }
...
//     "inference": "Business & Industrial",
//     "title": "Home - SmallNetBuilder",
//     "date": "2021-09-07 Tue Sep 07 2021 23:18:00 GMT-0400 (Eastern Daylight Time)",
//     "real": "2021-09-07 23:18:21",
//     "count": 4.921
//   }
// ]
...
//https://codesandbox.io/s/gyfnc?file=/src/styles.css
//https://nick-romano.github.io/react-hour-heatmap/props
        // data={[
        //   {date: new Date("8.31.2021 01:51"), count: 25, title: "cool routers", domain: "smallnetbuilder", inference: "news"}, // count should be seconds on website
        //   {date: new Date("8.31.2021 01:50"), count: 30, title: "expensive routers", domain: "smallnetbuilder", inference: "heat"}, // count should be seconds on website
        //   {date: new Date("8.31.2021 14:51"), count: 4, inference: "heat waves", title: "music searching"  },
        //   {date: new Date("8.31.2021 14:51"), count: 12, inference: "heart", title: "music searching"   },
        //   {date: new Date("8.31.2021 14:52"), count: 12, inference: "heart", title: "music searching"   },
        //   {date: new Date("8.31.2021 16:23"), count: 22, inference: "messaging", title: "music searching"   },
        //   {date: new Date("8.31.2021 16:51"), count: 102, inference: "applications", title: "music searching"   }
        // ]}


                        // details of each: {JSON.stringify(heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(item => item.details))}
                        //                test: {JSON.stringify(heatmap.data)}
                        // Pages Visited: {JSON.stringify(heatmap.data.minutes.filter(r => r.data.details).map(entry => {entry.total_webpage_count}))} 


                        // backup:  Interests: {JSON.stringify(heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => entry.interest_overview).reduce(set_combine, new Object())     )  }
                        // .map(n=>Object.keys(n))
                        //                 {heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => entry.interest_overview).reduce(set_combine, new Object(), 0 , 0 ).map(item => <ul><li> {item[0]} : {fancyTimeFormat(item[1])} </li></ul>)      }  


                        // entry.interest_overview.length
                        //                 {heatmap.data.minutes.map(r => r.data).filter(r => r.count > 0).map(entry => (entry.interest_overview.length == 0) ? ( [0] ) : (entry.interest_overview)).reduce(set_combine, new Object(), 0 , 0 ).map(item => <ul><li> {item[0]} : {fancyTimeFormat(item[1])} </li></ul>)      }  
...
const heatMap_grade = (allData) => {
  ...
  if (numEntries === 0) {
    ...
    return (
            ...
            <div>
              <Spinner title='crunching the numbers…' size='small' />
            </div>
...
const heatMap_overview = (slice, allData) => {
  ...
  if (numEntries !== 0) {
    ...
    if (allData.all.length == 0) {
      return (<p>Trackers get hungry too, visit some websites and feed them data!</p>)
    } else {
      ...
      let top_time = millisToReadable(all_activity_in_slice[top_interest_revised])
      console.log("dogsnice" + JSON.stringify(all_activity_in_slice))
      ...
      try {
        ...
        return (<p>Top-Time Interest: <b>{top_interest_revised}</b><br/>Engagement: <b>{top_time}</b></p>)
      } catch (e) {
        return (<p>Top-Time Interest: --pending--<br/>Engagement: --pending--</p>)
      } 
...
const heatMap = (slice, allData) => {
  ...
  if (numEntries === 0) {
    ...
    //
    console.log("coming from heatMap ")
    console.log(allData.slice)

    let to_render = allData.slice

    let total_labels = new Object()
    total_labels.size = 8
...
const googleAdsSettings2 = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    return (
      ...
      <div id="treeWrapper" style={{ height: String(dataLength * 3) + "em",}}>
        <Tree data={data}
...
const googleAdsSettings3_deepest = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="15px">{item}</Text_grommet>)} data={deepest_interest.map(entry => entry.split("/")[deepest])}/>
...
const ads_overview_breakDown = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    let at_most = 10
    to_render = to_render.sort((a, b) => (a.count > b.count) ? 1 : -1).reverse()
    ...
    <List_grommet primaryKey={item => (<Text_grommet size="13px">{item.category}</Text_grommet>)} secondaryKey={item => (<Text_grommet size="11px">{item.count}</Text_grommet>)} data={to_render}/>     
    )
  }
...
const googleAdsSettings3_deepest_totals = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    return  (
      ...
      <Box direction="column" alignSelf='center' align="center" pad={{ bottom: 'xsmall' }}>
        <Text_grommet size="100px" weight="bold">{data['total_counts']}</Text_grommet>
        <br/>
        <Text_grommet pad="medium" align="center" color="dark-3" margin="medium" size="small">Total count of interests (excluding demographics)</Text_grommet>
      </Box>
...
const ads_overview_totalCount_count = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    return  (
      ...
      <Box direction="column" alignSelf='center' align="center" pad={{ bottom: 'xsmall' }}>
        <Text_grommet size="100px" weight="bold">{data['totals']}</Text_grommet>
        <br/>
        <Text_grommet pad="medium" align="center" color="dark-3" margin="medium" size="small">Total count of ads targeted to you</Text_grommet>
      </Box>
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    return  (
      ...
      <Stack anchor="top-right">
        <Box alignSelf="center" align="center" border={{ color: 'accent-1', size: 'medium' }} round='medium' pad="small" margin="medium" gap="small" width={{ max: 'medium' }} responsive={true}> 
        <Text_grommet size='5xl' weight='bold'>{'$' + ((data['totals']) * 1.58).toFixed(2)}</Text_grommet>
        </Box> 
        <br/>
        <Tip plain content={  <Box background='light-1' round='medium' pad="small" margin="small" gap="small" width={{ max: 'medium' }} responsive={false} > <Text_grommet weight="bold" color="status-error">Estimated Cost</Text_grommet> <Text_grommet size="small"> Advertisers pay to serve you ads. Typically, this pricing is dependent on many factors, including click-counts. Here, we take the average cost of displaying an advertisement ($1.58) multiplied by the number of ads you've been served. </Text_grommet> </Box> } dropProps={{ align:  { top: "bottom" } }}>
    ...
    const options = {
      ...
      tooltip: {
        ...
        formatter: function() {
          ...
          let ret = String(moment(this_entry.date).format("dddd, MMMM Do, hh:mm a"))
          ret += '</br></br><b>' + this_entry.y + ' Interests </b>'
          if (this_entry['diff'].length != 0) {
            ret += '<br/><br/>' + "New:" + "<br/>" + this_entry.diff.map(entry =>entry.value.split("/")[entry.value.split("/").length-1]) 
          }
    ...
    const options = {
      ...
      tooltip: {
        ...
        formatter: function() {
          ...
          // let ret = JSON.stringify(this_entry)
          let ret = String(moment(this_entry.date).format("dddd, MMMM Do, hh:mm a"))  //entry.value.split("/")[entry.value.split("/").length-1]
          ret += '</br></br><b>' + this_entry.y + ' Interests </b>'
          if (this_entry['diff'].length != 0) {
            ...
            for (let interest of this_entry.diff) {
              if (interest.type == "interest - company") {
                ...
                ret += interest.value.split("/")[interest.value.split("/").length-1] + ", "
              }
            }
            ret = ret.slice(0, ret.length-2) // remove comma plus space at end

            if (this_entry['pages_visited'].length != 0) {
              ret += '<br/><br/>' + "<b>Pages visited around this time:</b>" + "<br/>" + "<ul>" + this_entry.pages_visited.map(entry =>"<li>"+entry+"</li>").join("") + "</ul>"
            }
          }
...
const bedTimes_version2 = (data) => {
  ...
  [
    1634099380052,
    new Date(1634099380052).getHours() + 24
  ],
  [
    1634174749441,
    new Date(1634174749441).getHours()
  ],
  [
    1634099381052,
    new Date(1634099381052).getHours() + 24
  ],
  [
    1634175669188,
    new Date(1634175669188).getHours()
  ],
  [
    1634177482596,
    new Date(1634177482596).getHours()
  ],
  [
    1634178241669,
    new Date(1634178241669).getHours()
  ],
  [
    1634180550522,
    new Date(1634180550522).getHours()
  ],  [
    1634257349212,
    new Date(1634257349212).getHours()
  ],  [
    1634263982732,
    new Date(1634263982732).getHours()
  ],  [
    1634270485815,
    new Date(1634270485815).getHours() + 24
  ], 
  [
    1634270485815,
    24,
    ...
    for (let time_entry of data) {
      ...
      if (peak_time !== "undefined") {
        ...
        let actual_minutes = parseInt(String(((parseFloat(minutes)*100) + 1)%60).slice(0.2))
        ...
        if (parseInt(hours) % 24 <= 5 && parseInt(hours) % 24 > 0) {
          average_pretty_print = String(parseInt(hours) % 24) + ":" + String(actual_minutes) + " AM"
        } else {
          average_pretty_print = String(hours - 12) + ":" + String(actual_minutes) + " PM"
        }
    ...
    if (peak_inferences.length != 0) {
      ...
      let sorted = _.fromPairs(_.sortBy(_.toPairs(inference_counts), 1).reverse())
      ...
      engagement_str = '<p style="font-size: 20;"> Go-To Late-Night Interest: <strong>' + top_activity + '</strong> (<em>happened ' + engagement + " " + engagement_saying + '</em>)</p>'
    } else {
    let engagement_str = ''
    ...
    const options = {
      ...
      yAxis: {
        ...
        plotLines: [{
            ...
            label: {
                text: "<p style='color:red'>Late Night<br/>Average <br/>(" + average_pretty_print + ")</p>",
                align: 'top',
                x: -10
            }
        }],
      ...
      tooltip: {
          formatter: function() {  //return ( moment(this.x).format("dddd, MMMM Do YYYY, h:mm:ss a") + '<br/>' + point)
                var serie = this.series;
                //NOTE: may cause efficiency issue when we got lots of points, data in series
                //should be change from [x, y] to {"x": x, "y": y, "index": index}
                var index = this.series.data.indexOf(this.point);
...
const bedTimes_version3 = (data) => {
  ...
  if (numEntries == 0) {
    ...
    var options = {
      ...
      title: {
        text: 'Trackers know when you go to sleep at night'
      },
      subtitle: {
        text: document.ontouchstart === undefined ?
          'Click and drag in the plot area to zoom in' : 'Pinch the chart to zoom in'
      ...
      yAxis: [{
        ...
        // plotLines: [{
        //     color: 'red',
        //     value: data[2]['latest_nights'], // Insert your average here
        //     width: '1.5',
        //     zIndex: 4, // To not get stuck below the regular plot lines or series
        //     label: {
        //         text: "<p style='color:red'>Late Night<br/>Average</p>",
        //         align: 'right',
        //         x: -10
        //     }
        // }],
...
const demographics = (data) => {
  ...
  if (numEntries === 0 || dataLength == 0) {
    ...
    <div>
    Please use this link to log in and import your Google Ads Settings information! Please note, if you have ad personalization turned off, nothing will show below. 
    ...
    let renderable = data['children'].map(entry => entry.name).toString()
    renderable = renderable.replaceAll(",", "\n")
...
const demographics_version2 = (demograhpics, contact) => {
  ...
  if (demo_dataLength == 0 || contact_numEntries == 0 || contact_dataLength == 0) {
    return (
    ...
    let render_names = my_name.map(function(item){ return item.my_name; });
    ...
    let render_emails = my_emails.map(function(item){ return item.my_email; });
    ...
    // ////////////// jobs
    // let my_job = demograhpics['children'].filter(entry=>entry['name'].includes("Job Industry:"))
    // let render_jobs = my_job.map(function(item){ return item.name; });
    // let render_jobs_string = ''
    // let jobs_length = render_jobs.length
    // render_jobs.forEach(function (item, index) {
    //   if (index !== jobs_length-1) {
    //     render_jobs_string += item + " & "
    //   } else {
    //     render_jobs_string += item 
    //   }
    // });
    ...
    // ////////////// languages
    // let my_language = demograhpics['children'].filter(entry=>entry['name'].includes("Language:"))
    // let render_languages = my_language.map(function(item){ return item.name; });
    // let render_languages_string = ''
    // let languages_length = render_languages.length
    // render_languages.forEach(function (item, index) {
    //   if (index !== jobs_length-1) {
    //     render_languages_string += item + " & "
    //   } else {
    //     render_languages_string += item 
    //   }
    // });
    ...
    // <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>

    // <Stack anchor="bottom-right" alignSelf="center" margin={{top: "20px"}}>
    // <Gift size="large" />
    // {icon_add_in}
    // </Stack>
    ...
    // <Text_grommet alignSelf="center" size="xsmall"> Age </Text_grommet>
    ...
    console.log(myArr)
    let myMatrix = myArr.reduce((rows, key, index) => (index % 3 == 0 ? rows.push([key]) : rows[rows.length-1].push(key)) && rows, []);
    ...
    for (let row of myMatrix) {
      ...
      for (let column of row) {
        let entry = { name: "box-" + String(total_count), start: [column_count, row_count], end: [column_count, row_count] }
    ...
    for( let entry of demograhpics['children']) {
      ...
      if (emails_length > 1) {
        ...
        icon_add_in =       <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation="slideRight" responsive={false}><Text weight="bold">Attribute Associated With</Text><Text size="small"><ul>{source.map(entry => <li> {entry} </li>)}</ul></Text></Box>} dropProps={{ align: { left: 'right' } }}>
                            <Box
                            background="brand"
                            pad={{ horizontal: 'xsmall' }}
                            round
                            >
                            <Text_grommet size="xxsmall">{source.map(email => "*" + email_mapping[email])}</Text_grommet>
                            </Box>
                            </Tip>
      } else {
        icon_add_in = ''
      ...
      if (value.includes(" years old")) {
        ...
        let entry = 
                  ...
                  <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks you are</Text_grommet><Text_grommet color="accent-2" size="small">{age}</Text_grommet><Text_grommet size="small" weight="bold">Options for age targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["18-24", "25-34", "35-44", "45-54", "55-64", "65 or more", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon language
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>

                      <Stack anchor="bottom-right" alignSelf="center" margin={{top: "100px"}}>
                      <Globe size="xlarge" />
                      {icon_add_in}
                      </Stack>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Language </Text_grommet>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// meter company size
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'> 
                    ...
                    <Text_grommet alignSelf="center" size="xsmall"> Employer Size </Text_grommet>

                    <Box alignSelf="center" height="large" >
                    <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks you work at a</Text_grommet><Text_grommet color="accent-2" size="small">{company}</Text_grommet><Text_grommet size="small" weight="bold">Options for company size targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Small Employer (1-249 Employees)", "Midsize Employer (250-999 Employees)", "Large Employer (1k-10k Employees)", "Very Large Employer (10k+ Employees)"]}/></Box>} dropProps={{ align: slide_direction }}>
                    <CircleInformation color="light-4" size="medium"/>
                    </Tip>
                    </Box>
                    </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// meter education
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'> 
                    ...
                    <Text_grommet alignSelf="center" size="xsmall"> Education Status </Text_grommet>

                    <Box alignSelf="center" height="large" >
                    <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your highest level of education is</Text_grommet><Text_grommet color="accent-2" size="small">{education}</Text_grommet><Text_grommet size="small" weight="bold">Options for education level targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["High School Graduate", "Bachelor’s Degree", "Advanced Degree"]}/></Box>} dropProps={{ align: slide_direction }}>
                    <CircleInformation color="light-4" size="medium"/>
                    </Tip>
                    </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon homeowner
        ...
        if (is_not==false && unknown==false) {
          entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Homeownership Status </Text_grommet>


                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks you are a</Text_grommet><Text_grommet color="accent-2" size="small">{homeowner.split("s")[0]}</Text_grommet><Text_grommet size="small" weight="bold">Options for homeownership status targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Homeowner", "Renter", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
        ...
        if (unknown==true) {
          entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google is unsure about your homeownership status</Text_grommet><Text_grommet color="accent-2" size="small">{homeowner}</Text_grommet><Text_grommet size="small" weight="bold">Options for homeownership status targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Homeowner", "Renter", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon household income
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Household Income </Text_grommet>

                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your income level is</Text_grommet><Text_grommet color="accent-2" size="small">{household}</Text_grommet><Text_grommet size="small" weight="bold">Options for income level targeting include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Top 10%", "11-20%", "21-30%", "31-40%", "41-50%", "Lower 50%", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon job industry
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Job Industry </Text_grommet>

                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your job industry is</Text_grommet><Text_grommet color="accent-2" size="small">{job}</Text_grommet><Text_grommet size="small" weight="bold">Options for job industry include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Construction", "Education", "Financial", "Healthcare", "Hospitality", "Manufacturing", "Real Estate", "Technology", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon gender
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Gender </Text_grommet>

                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your gender is</Text_grommet><Text_grommet color="accent-2" size="small">Male</Text_grommet><Text_grommet size="small" weight="bold">Options for gender include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Male", "Female", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon gender
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your gender is</Text_grommet><Text_grommet color="accent-2" size="small">Female</Text_grommet><Text_grommet size="small" weight="bold">Options for gender include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Male", "Female", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon gender
        ...
        let entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your gender is</Text_grommet><Text_grommet color="accent-2" size="small">Unknown</Text_grommet><Text_grommet size="small" weight="bold">Options for gender include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Male", "Female", "Unknown"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon marital status
        ...
        if (is_not==false && unknown==false) {
          entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Marital Status </Text_grommet>

                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your marital status is</Text_grommet><Text_grommet color="accent-2" size="small">{marital}</Text_grommet><Text_grommet size="small" weight="bold">Options for marital status include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Single", "In a Relationship", "Married"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////////// icon parental status
        ...
        if (is_not==false && unknown==false) {
          entry = <Box gridArea={"box-" + String(counter)} background='light-1' round='medium'>
                      ...
                      <Text_grommet alignSelf="center" size="xsmall"> Parental Status </Text_grommet>

                      <Box alignSelf="center" height="large" >
                      <Tip plain content={<Box pad="small" gap="small" width={{ max: 'large' }} round="small" background="background-front" animation={animation_direction} responsive={false}><Text_grommet weight="bold">Google thinks your parental status is</Text_grommet><Text_grommet color="accent-2" size="small">{parental}</Text_grommet><Text_grommet size="small" weight="bold">Options for parental status include</Text_grommet><List_grommet primaryKey={(item) => (<Text_grommet key={item} size="12px">{item}</Text_grommet>)} data={["Not a parent", "Parents of Infants (0-1 years)", "Parents of Toddlers (1-3 years)", "Parents of Preschoolers (4-5 years)", "Parents of Grade-Schoolers (6-12 years)", "Parents of Teens (13-17 years)"]}/></Box>} dropProps={{ align: slide_direction }}>
                      <CircleInformation color="light-4" size="medium"/>
                      </Tip>
                      </Box>
...
const advertiser_ads = (data) => {
  ...
  if (numEntries === 0) {
    return (
      ...
      //   let obj = {dom: dom, domain: domain, pageId: pageId, explanation: explanation}
      //   all_ads.push(obj)
      // }
      ...
      //   // let str = '<div><iframe srcDoc={' + all_ads[i].dom + '/></div>'
      //   console.log(all_ads[i])
      // }
        ...
        // console.log("fourth" + data[3])
        //     //
        // return (
        //   <Carousel 
        //   > 
        //     <div>
        //     <iframe srcDoc={data[0]} sandbox=""/>
        //     </div>
        //     <div>
        //     <iframe srcDoc={data[1]} sandbox=""/>
        //     </div>
        //     <div>
        //     <iframe srcDoc={data[2]} sandbox=""/>
        //     </div>
        //     <div>
        //     <iframe srcDoc={data[3]} sandbox=""/>
        //     </div>
        //   </Carousel>
      ...
      const highlighted_domain_style = { color: 'lightblue', fontWeight: 'bold', textDecoration: "underline" }
      const highlighted_inference_style = { color: 'lightred', fontWeight: 'bold' }
      const list_style = { textAlign: 'left'  }

// <span style={highlighted_domain_style}>{page_data.domain}</span>
//                <div> <iframe srcDoc={entry.dom} style={frame_style}/><p className="legend">You visited <span style={highlighted_domain_style}>{entry.domain}</span> and saw this ad, realted to <span style={highlighted_inference_style}>{entry.inference}</span> <br/><br/> <span style={list_style}>  { entry.pages.map(page_data => ( <ul><li> "{page_data.title}"  is related <ul><li> you visited this site on {moment(page_data.id).format("dddd, MMMM Do YYYY, h:mm a")} </li></ul> </li></ul> ) ) } </span></p></div>

  //<span style={highlighted_inference_style}>{entry.inference}</span>
        ...
        return (
            ...
            >
              {data.map(entry => (
                <div> <iframe srcDoc={entry.dom} style={frame_style}/><p className="legend">You're interested in <span style={highlighted_inference_style}>{entry.inference}</span> ←←← which is why we think you saw this ad <br/><br/> <span style={list_style}>  { entry.pages.map(page_data => ( <ul><li>[related] <i>{page_data.title}</i> 😉 ({moment(page_data.id).format("dddd, MMMM Do YYYY, h:mm a")}) </li></ul> ) ) } </span></p></div>
      ...
      //   //   <Carousel> 
      //   //     <div>
      //   //     <iframe srcDoc={obj.dom} sandbox=""/>
      //   //     <p className="legend">Legend 8</p>
      //   //     </div>
      //   //     <div>
      //   //     <iframe srcDoc={"<html><head></head><body><h1>Hello World<h1></body></html>"} sandbox=""/>
...
// const allItems = Array(50).fill().map((_, i) => i + 1 + "akljsdflajskdflajsdfjalksdjf lajsdlfjasldjfla   www.googe.com sjdlfjasldjfalsjdfljasdlfjalsdjflajsdfljkasdflkjasdf");

const advertiser_ads_version2 = (type, data_all, current_slide, slide_updater) => {
    ...
    return (
      ...
      for (let this_entry of data) {
        ...
        if (this_entry.ad_category_matches_other_pages.length != 0) {
          ad_category_matches_other_pages = this_entry.ad_category_matches_other_pages
          count_of_all += 1
          mapping['ad_category_matches_other_pages'] = count_of_all
        }
        if (this_entry.ad_category_matches_google_interests.length != 0) {
          ad_category_matches_google_interests = this_entry.ad_category_matches_google_interests
          count_of_all += 1
        ...
        if (this_entry.ad_domain_exactly_seen_in_history.length != 0) {
          ad_domain_exactly_seen_in_history = this_entry.ad_domain_exactly_seen_in_history
          count_of_all += 1
          mapping['ad_domain_exactly_seen_in_history'] = count_of_all
        }
        if (this_entry.ad_domain_fuzzy_seen_in_history.length != 0) {
          ad_domain_fuzzy_seen_in_history = this_entry.ad_domain_fuzzy_seen_in_history
          // count_of_all += 1
      ...
      console.log(this_entry)
        ...
        if (this_entry.ad_category != 'none') {
          ...
          let entry = <Box gridArea={"box-" + String(mapping['ad_category'])} background='light-6' round='medium'>

                        <Box alignSelf='center' margin={{top: "20px",}}>
                        <ContactInfo color='black' size="medium" />
                        </Box>
                        <Text_grommet alignSelf="center" size="medium"> The Ad's Category</Text_grommet>
                        <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={[this_entry.ad_category]}/>
                        <br/>
                        </Box>
        ...
        if (this_entry.ad_domain_mod != '') {
          ...
          let entry = <Box gridArea={"box-" + String(mapping['ad_domain_mod'])} background='light-6' round='medium'>

                        <Box alignSelf='center' margin={{top: "20px",}}>
                        <Domain color='black' size="medium" />
                        </Box>
                        <Text_grommet alignSelf="center" size="medium"> The Ad's Owner</Text_grommet>
                        <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={[this_entry.ad_domain_mod]}/>
                        <br/>
                        </Box>
          grid_entries.push(entry)
        }
        ...
        if (this_entry.ad_category_matches_google_interests.length != 0) {
          ...
          let entry = <Box gridArea={"box-" + String(mapping['ad_category_matches_google_interests'])} background='light-6' round='medium'>

                        <Box alignSelf='center' margin={{top: "20px",}}>
                        <Google color='black' size="medium" />
                        </Box>
                        <Text_grommet alignSelf="center" size="medium"> Google says you're interested in</Text_grommet>
                        <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={keepers}/>
                        <br/>
                        <Text_grommet alignSelf="center" color='dark-4' size="medium"> (top 3)</Text_grommet>
                        </Box>
          grid_entries.push(entry)
        }
        ...
        if (this_entry.ad_category_matches_other_pages.length != 0) {
          ...
          let entry = <Box gridArea={"box-" + String(mapping['ad_category_matches_other_pages'])} background='light-6' round='medium'>

                        <Box alignSelf='center' margin={{top: "20px",}}>
                        <History color='black' size="medium" />
                        </Box>
                        <Text_grommet alignSelf="center" size="medium"> Similar interests in history</Text_grommet>
                        <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={keepers}/>
                        <br/>
                        <Text_grommet alignSelf="center" color='dark-4' size="medium"> (top 3)</Text_grommet>
                        </Box>
        ...
        if (this_entry.ad_domain_exactly_seen_in_history.length != 0) {
          ...
          console.log("arry" + JSON.stringify(this_entry.ad_domain_exactly_seen_in_history))
          ...
          let entry = <Box gridArea={"box-" + String(mapping['ad_domain_exactly_seen_in_history'])} background='light-6' round='medium'>

                        <Box alignSelf='center' margin={{top: "20px",}}>
                        <History color='black' size="medium" />
                        </Box>
                        <Text_grommet alignSelf="center" size="medium"> Similar webpage visits</Text_grommet>
                        <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={keepers}/>
                        <br/>
                        <Text_grommet alignSelf="center" color='dark-4' size="medium"> (top 3)</Text_grommet>
                        </Box>
        ...
        if (this_entry.ad_explanation != null) {
          if (this_entry.ad_explanation[0] != 'none provided by advertiser') {
            ...
            let entry = <Box gridArea={"box-" + String(mapping['ad_explanation'])} background='light-6' round='medium'>

                          <Box alignSelf='center' margin={{top: "20px",}}>
                          <Info color='black' size="medium" />
                          </Box>
                          <Text_grommet alignSelf="center" size="medium"> Provided ad explanation</Text_grommet>
                          <List_grommet primaryKey={(item) => (<Text_grommet key={item} size="11px">{item}</Text_grommet>)} data={keepers}/>
                          <br/>
                          <Text_grommet alignSelf="center" color='dark-4' size="medium"> (top 3)</Text_grommet>
                          </Box>
        ...
        let grid_entry = 
                  ...
                  >
                    ...
                    <Box gridArea='head' gap='xsmall' background='none' pad="xsmall" margin='xsmall' round='large'> 
                      ...
                      <Text_grommet color="dark-4" size='13px' alignSelf='center'> visited by you ({String(moment(this_entry.on).format("dddd, MMMM Do YYYY, h:mm:ss a"))}) </Text_grommet>
                    </Box>
                  ...
                  <Box flex={true} full={true} align="center" alignSelf="center" background='none' round='medium' pad="medium" full={true} margin="xsmall" gap="xsmall" width='xxlarge'>
                  {<iframe srcDoc={this_entry.dom} style={frame_style} frameBorder="0"/>}
                  </Box>
                  </Box>


        explanations_grid.push(<Box round='small' pad='medium' border={{ color: 'light-3', size: 'small' }}> {grid_entry} </Box>)
        ...
        return (
          ...
          <Grommet>
            <Box align="center" direction='column' pad="small">
                      ...
                      <Box margin='medium'>
                      <Text_grommet>
                      {current_slide+1 <= 1 && <Button_grommet disabled={true} label="previous" />} 
                      {current_slide+1 != 1 && <Button_grommet label="previous" onClick={() => slide_updater(current_slide - 1)} />}
                      <Text_grommet> {current_slide+1} (of {data.length}) </Text_grommet>
                      {current_slide+1 != data.length && <Button_grommet label="next" onClick={() => slide_updater(current_slide + 1)} />}
                      {current_slide+1 >= data.length && <Button_grommet disabled={true} label="next" />} 
                      </Text_grommet>
                      </Box>

              <Carousel_grommet initialChild={0} controls={false} fill={true} activeChild={current_slide} onChild={slide_updater}>
...
// cut time beforehand to send to component
const sliceData = (data, slice) => {

  let originalData = data.original

  console.log("within sliceData")
  ...
  for (let i = 0; i < Object.keys(originalData).length; i++) {
      ...
      for (let r = 0; r < Object.keys(originalData[i].details).length; r++) {


        let new_date_date = originalData[i].details[r].date.split(" ")[0]
        let new_date_time = originalData[i].details[r].date.split(" ")[1]
        ...
        let within_month = moment(new_date_date).isAfter(one_month_ago);
        let within_week = moment(new_date_date).isAfter(one_week_ago);
        let within_day = moment(new_date_date).isAfter(one_day_ago);
        let is_today = (new_date_date == date_today_formatted)

        console.log(new_date_date + " is within one week " + within_week + " is within one month " + within_month + " is within one day " + within_day + " is today " + is_today)


        // let new_date = moment(date_today_formatted).subtract(15, 'days').format('YYYY-MM-DD')
        // let test_within_month = moment(new_date).isAfter(one_month_ago);
        // let test_within_week = moment(new_date).isAfter(one_week_ago);
        // let test_within_day = moment(new_date).isAfter(one_day_ago);

        // console.log("testing" + new_date + " is within one week " + test_within_week + " is within one month " + test_within_month + " is within one day " + test_within_day)


        if (slice=='all') {

          all_totals += originalData[i].details[r].value

          // keep track of most common items 
          if (originalData[i].details[r].value > highest_time) {
            highest_time = originalData[i].details[r].value
            highest_category = originalData[i].details[r].name
          }

          all_details.push({name: originalData[i].details[r].name, date: date_today_formatted + " " + String(new_date_time), value: originalData[i].details[r].value, real_time: originalData[i].details[r].date})

          total_labels.add(originalData[i].details[r].name)

          console.log("all --" + new_date_date)
        }
          ...
          console.log("within_month and last month --" + new_date_date)
          ...
          console.log("within week and last week --" + new_date_date)
          ...
          all_details.push({name: originalData[i].details[r].name, date: date_today_formatted + " " + String(new_date_time), value: originalData[i].details[r].value, real_time: String(originalData[i].details[r].date + "--" + slice)})

          total_labels.add(originalData[i].details[r].name)

          console.log("is today and today --" + new_date_date )
  ...
  let ret = {slice: all_days, original: originalData}
  console.log("final product from sliceData")
  console.log(ret)
  return (ret)
...
constructor (props) {
    ...
    this.state = {
      ...
      allData: [],
      numTrackers: '…',
      myData : [{"_id": "this", "value": 1, "colorValue": '#FFFF', "selected": "the other"}, {"_id": "this", "value": 3, "colorValue": '#FFFF', "selected": "the other"}]
    }
  ...
  async getData () {
    ...
    const heat_map_newer = background.queryDatabase('PageIdDataStructure_revisedHeatmap', {})
    ...
    const jobTopicPages = background.queryDatabase('getTopicPages',{topics: jobTopics,excluded: ['Lose']})
    const jobTopicSearchPages = background.queryDatabase('getTopicSearchPages',{topics: jobTopics,excluded: ['lose']})
    ...
    const sensitive_info = background.queryDatabase('getInferencesMostSensitive', {}) 
    const sensitive_info_v2 = background.queryDatabase('getInferencesMostSensitive_version2', {})
    const sensitive_info_v3 = background.queryDatabase('getInferencesMostSensitive_version3', {})
    const sensitive_info_v4 = background.queryDatabase('getInferencesMostSensitive_version4', {})
    const sensitive_info_bubbles = background.queryDatabase('getInferencesMostSensitive_bubbles', {})
    ...
    const bedtime_v3 = background.queryDatabase('getPagesByTime_bedtime_version2', {})
    ...
    heat_map_newer.then(n => this.setState({heat_map_newer: n}))
    ...
    sensitive_info.then(n => this.setState({sensitive_info: n}))
    sensitive_info_v2.then(n => this.setState({sensitive_info_v2: n}))
    sensitive_info_v4.then(n => this.setState({sensitive_info_v4: n}))
    sensitive_info_bubbles.then(n => this.setState({sensitive_info_bubbles: n}))
    sensitive_info_bubbles_v2.then(n => this.setState({sensitive_info_bubbles_v2: n}))
    bedtime_v2.then(n => this.setState({bedtime_v2: n}))
    bedtime_v3.then(n => this.setState({bedtime_v3: n}))
    ...
    heat_map.then(n => { 
      ...
      let keep_original = {slice: n, original: n}

      let best = sliceData(keep_original, this.state.graphCount)
      console.log(best)
  ...
  updatePieCount (event) {
    ...
    this.setState({
      pieCount: new_type,
            ...
            // {heatMap(this.state.graphCount, this.state.allData)}
  ...
  render () {
    ...
    const { numPages, bedtime_v2, bedtime_v3, adDOM_overview, google_contactInfo, getTopicsOfInterest, sensitive_info, sensitive_info_v2, sensitive_info_v3, sensitive_info_v4, sensitive_info_bubbles, sensitive_info_bubbles_v2, myData, noTrackers, manyTrackers, google_demographics, weektimestamps, recent, searchPages, jobTopicPages, jobTopicSearchPages, relTopicPages, relTopicSearchPages, pagesByTime, graphCount, ad_type, carousel_slide_count, google_interests, google_interests_bars, numDomains, allTitles, allAds, allAdURLs, allAdDOMs, allGoogleInferences, adExplanation_counts, ad_deep, heat_map, heat_map_newer, heat_map_newer_newer} = this.state
    ...
    // console.log("jobTopicPages" + JSON.stringify(this.state.jobTopicPages))
    // console.log("job topic search" + JSON.stringify(this.state.jobTopicSearchPages))
    // console.log("relationship topic" + JSON.stringify(this.state.relTopicPages))
    // console.log("relationship search" + JSON.stringify(this.state.relTopicSearchPages))
        ...
        // <GridRow>
        //   <GridCol>
        //     <Heading level='h1'><strong>Your Profile</strong></Heading>
        //   </GridCol>
        // </GridRow>
        ...
        //       <Text>
        //         <p>
        //           Tracking Transparency is a free browser extension that helps you uncover you digital self &mdash; the treasure trove of digital inforamtion that companies amass about you as you browser the web. This data comes from...
        //         </p>
        //       </Text>
        //       <video loop autoPlay>
        //         <source src="places.mp4" type="video/mp4" />
        //         <source src="places.mp4" type="video/ogg" />
        //         Your browser does not support the video tag.
        //       </video>
        //       <Text>
        //         <p>
        //           <FontAwesomeIcon icon='eye' /> &#160; the webpages you visit
        //         </p>
        //         <p>
        //           <FontAwesomeIcon icon='clock' /> &#160; the time you spend on those webpages 
        //         </p>
        //         <p>
        //           <FontAwesomeIcon icon='lightbulb' /> &#160; inferences drawn about you given the content of those webpages 
        //         </p>
        //       </Text>
        //     </TTPanel>
        //   </GridCol>
        ...
        //       <GridCol >
        //         <br/>
        //         <Heading level='h1'><strong>Burning the midnight oil?</strong></Heading>
        //         <br/>
        //         {bedTimes_version3(this.state.bedtime_v3)}
        //       </GridCol>
        //     </TTPanel>
        //   </GridCol>
        // </GridRow>
        ...
        //       <Heading level='h1'><FontAwesomeIcon icon='clock' /><strong>&nbsp; Burning the midnight oil?</strong></Heading>
        //     {ok && <div>
        //       <TTPanel margin='medium 0 medium 0'>
        //         <BedTimeGraph
        ...
        //     <GridRow>
        //       <GridCol>
        //         <Heading level='h1'><FontAwesomeIcon icon='window-maximize' /><strong>&nbsp; Looking for...</strong></Heading>
        //           <br/>
        //           {searchesString(uniqueJobTopicSearchPages, " [job hunt]")}
        //           <br/>
        ...
        //       <GridCol>
        //           <br/>
        //           {searchesString(uniqueRelTopicSearchPages, " [relationships]")}
        //           <br/>
        ...
        //   <GridCol>
        //     <TTPanel>
        //       <Heading level='h1'><strong>oof, that could be sensitive</strong>&nbsp; 😊 (but thanks for sharing) 😊</Heading>
        //       <br/>
        //       {sensitive_table(sensitive_info)}
        //       <br/>
        //       {sensitive_info_pie(this.state.sensitive_info_v2)}
        //       <br/>
        //       {sensitive_bubbles(sensitive_info_bubbles)}
        //     </TTPanel>
        //   </GridCol>
        // </GridRow>

        ////////////////////////////////////////////// turn me back on but i'm slow 
        ////////////////////////////////////////////// turn me back on but i'm slow 
        ////////////////////////////////////////////// turn me back on but i'm slow 
        ////////////////////////////////////////////// turn me back on but i'm slow 
        ////////////////////////////////////////////// turn me back on but i'm slow 
        ...
        //     <TTPanel>
        //       <Text>
        //       <Heading level='h1'><strong>Personalized Ads are fun!</strong></Heading>
        //       </Text>
        //       <br/>
        //       <GridCol >
        //         {advertiser_ads(allAdDOMs)}
        //       </GridCol>
        //     </TTPanel>
        //   </GridCol>
        // </GridRow>
        ...
        //   <GridCol>
        //     <TTPanel>
        //       <Heading level='h1'><strong>{heatMap_grade(this.state.heat_map_newer)} for Sharing!</strong></Heading>
        ...
        //       </GridCol>
        //       <GridCol>
        //         {heatMap_newer_radial(this.state.graphCount, this.state.heat_map_newer_newer)}
        //       </GridCol>
        //     </TTPanel>
        //   </GridCol>
        // </GridRow>
        ...
        // first attempt redoing sensitive interests
        // <br/>
        // {sensitive_bubbles(sensitive_info_bubbles)}
        // <br/>
        ...
        // </GridCol>
          ...
          //   responsive={false}
          // >
          //   <Text_grommet weight="bold" color="status-error">Google</Text_grommet>
          //   <Text_grommet size="small">
          //     Learns who you are and what you like
          //   </Text_grommet>
          // </Box>
  ...
  // // overall spinner function
  // let check_Demographics = this.state.google_demographics ? this.state.google_demographics['children'].length : 0
  // let check_adSettingsDeepest = this.state.google_interests ? this.state.google_interests['tree']['children'].length : 0
  // let check_adSettingsDeepest_bars = this.state.google_interests_bars ? this.state.google_interests_bars.length : 0
  // let check_adSettingsInterests = this.state.google_interests ? this.state.google_interests['tree']['children'].length : 0
  // let check_DOM = this.state.allAdDOMs ? this.state.allAdDOMs.length : 0

  // let all_pass = (check_Demographics != 0 && check_adSettingsDeepest != 0 && check_adSettingsDeepest_bars != 0 && check_adSettingsInterests != 0 && check_DOM != 0)

  // alert(this.state.google_demographics)



  console.log("letmeseeee" + google_demographics == null && google_contactInfo == null && google_interests == null)
  console.log("demo" + JSON.stringify(google_demographics))
  console.log("names" + JSON.stringify(google_contactInfo))
  console.log("interests" + JSON.stringify(google_interests))
    return (
      ...
      <Grid>
        ...
        <GridRow>
          <GridCol>
            ...
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Box alignSelf="center" align="center">
              <Stack anchor="top-right">
                <Box alignSelf="center" align="center" background='accent-2' round='medium' pad="small" margin="medium" gap="small" width={{ max: 'medium' }} responsive={true}> 
                <Text_grommet color="white">Who are you</Text_grommet>
                </Box>
        ...
        <GridRow>
          <GridCol>
            ...
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Stack anchor="top-right">
                <Box alignSelf="center" align="center" background='accent-2' round='medium' pad="small" margin="medium" gap="small" width={{ max: 'medium' }} responsive={true}> 
                <Text_grommet color="white"> What you like</Text_grommet> 
                </Box>
        ...
        <GridRow>
          <GridCol>
            ...
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Text_grommet alignSelf="center"> Time Slice to Aggregate </Text_grommet>
        ...
        {sensitive_info_v3 != null && sensitive_info_v3.all_pie.outer_all.length != 0 && 
        <GridRow>
          <GridCol>
            <Stack anchor="top-right">
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Text_grommet alignSelf="center"> Time Slice to Aggregate </Text_grommet>
              <Text_grommet size='12px'> 
              <RadioButtonGroup background='none' align='center' alignContent='start' alignSelf='center' direction='row' pad='xmall' margin='xsmall' flex overflow='scroll' name="slice" options={['all_pie', 'month_pie', 'week_pie', 'day_pie']} value={this.state.pieCount} onChange={this.updatePieCount} />
              </Text_grommet>
              </Box>
        ...
        <GridRow>
          <GridCol>
            ...
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Stack anchor="top-right">
                <Box alignSelf="center" align="center" background='accent-2' round='medium' pad="small" margin="medium" gap="small" width={{ max: 'medium' }} responsive={true}> 
                <Text_grommet color="white">Search habits</Text_grommet> 
                </Box>
                <Tip plain content={  <Box background='light-1' round='medium' pad="small" margin="small" gap="small" width={{ max: 'medium' }} responsive={false} > <Text_grommet weight="bold" color="status-error">Online engagement</Text_grommet> <Text_grommet size="small"> Trackers are able to look for popular search topics, like relationships and dating or diet.</Text_grommet> </Box> } dropProps={{ align:  { bottom: "top" } }}>
                <CircleInformation color='light-3' size="45px" />
                </Tip>
              </Stack>
              </Box>
        ...
        <GridRow>
          <GridCol>
            ...
            <Box background='white' round='small' pad="small" margin="small" gap="small" width={{ max: 'xxlarge' }} responsive={true}>
              ...
              <Stack anchor="top-right">
                <Box alignSelf="center" align="center" background='accent-2' round='medium' pad="small" margin="medium" gap="small" width={{ max: 'medium' }} responsive={true}> 
                <Text_grommet color="white">Ad overview</Text_grommet> 
                </Box> 
