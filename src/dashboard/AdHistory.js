...
const advertiser_ads_debug = (data) => {
  let numEntries = data ? data.length : 0
...
const advertiser_ads = (data) => {
  ...
  if (numEntries === 0) {
    return (
      ...
      {data.map(entry => (
        entry.pages.map(page_data => (console.log(page_data.domain + " visited on " + moment(page_data.id).format("dddd, MMMM Do YYYY, h:mm a"))))
      ))}
...
export class AdHistoryPage extends React.Component {
constructor (props) {
    super(props)
    this.state = {
      trackers: [],
  ...
  async getData () {
    ...
    const allAdDOMs = background.queryDatabase('getAdDOMs', {})
    ...
    allAdDOMs.then(n => this.setState({allAdDOMs: n}))
  ...
  updateGraphCount (event) {
    ...
    this.setState({
      graphCount: num,
            ...
            //{advertiser_ads2(allAds)}
  render () {

    const { numPages, noTrackers, google_demographics, graphCount, googleInterests, numDomains, allTitles, allAds, allAdURLs, allAdDOMs, allGoogleInferences, adExplanation_counts, ad_deep, heat_map} = this.state
    ...
    return (
      <Grid>
        ...
        <GridRow>
          <GridCol >
            <Heading level='h1'><strong>All Unique Ads</strong></Heading>
        ...
        <GridRow>
          <GridCol>
            <Heading level='h1'><strong>all ads (for debugging)</strong></Heading>
            {advertiser_ads_debug(allAds)}
          </GridCol>
        </GridRow>
