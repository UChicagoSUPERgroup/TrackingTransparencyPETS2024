...
class CalendarHeatmap extends React.Component {
  ...
  constructor(props) {
    ...
    this.total_labels_to_display = Math.max(this.props.total_labels, 7) 
    this.history = ['global']
    this.selected = {}
  ...
  calcDimensions() {
    ...
    this.settings.height = (this.settings.label_padding + this.total_labels_to_display * .75 * (this.settings.item_size + this.settings.gutter)) //pizza
  ...
  drawChart() {
    if ( this.overview === 'global' ) {
      ...
      // console.log(this.label_count + "pizza")
      // this.settings.height = this.settings.height + (this.label_count * 2) //pizza
  ...
  drawYearOverview() {
    ...
    this.items.selectAll('.item-circle')
      ...
      .attr('fill', d => {
        ...
        let formatted = moment(date_today).format("YYYY-MM-DD");
        if (d.date == formatted) {
          return 'yellow'
        } else {
      ...
      .on('mouseover', d => {
        ...
        let formatted = moment(date_today).format("YYYY-MM-DD");
        if (d.date == formatted) {
          tooltip_html += `<div class="${styles.header}"><strong>Today!</strong> <br><br> </div>`
        } 
  ...
  drawDayOverview() {
    ...
    // let n = this.props.total_labels
    // if (project_labels.length > n) {
    //   let keepers = project_labels.slice(0, n);
    //   let new_details = new Array() 

    //   // check if this "name" is in the keepers list
    //   for (let i = 0; i < Object.keys(this.selected.details).length; i++) {
    //     let element = this.selected.details[i]['name']
    //     let date = this.selected.details[i]['date']
    //     let value = this.selected.details[i]['value']
    //     if (keepers.includes(element)) {
    //       new_details.push({"name":element,"date":date,"value":value})
    //     }
    //   }
    //   this.selected.details = new_details;
    //   project_labels = keepers;
    // }

    let projectScale = d3.scaleBand()
      .rangeRound([this.settings.label_padding, this.settings.height]) //pizza
    ...
    this.items.selectAll('.item-block')
      ...
      .on('mouseover', d => {
        ...
        tooltip_html += `<div class="${styles.header}"><strong>${d.name}</strong><div><br>`
        tooltip_html += '<div><strong>' + (d.value ? this.formatTime(d.value) : 'No time') + ' tracked</strong></div>'
        tooltip_html += '<div>on  ' + moment(d.real_time).format('dddd, MMM Do YYYY HH:mm') + '</div>'
    ...
    this.labels.selectAll('.label-project')
      ...
      // show tooltip for what project this is
      .on('mouseover', d => {
        if (this.in_transition) { return }

        // Construct tooltip
        let tooltip_html = ''
        tooltip_html += '<div><span><strong>' + d + '</strong></span>'

        // Calculate tooltip position
        let x = this.settings.tooltip_padding * 2
        while (this.settings.width - x < (this.settings.tooltip_width + this.settings.tooltip_padding * 5)) {
          x -= 50
        }
        let y = this.settings.tooltip_padding * 3
